...
def blbl(keyword: str) -> dict[Any, Any]:
    ...
    return {"message": "搜索成功", "success": True, "data": data}
...
def db_server(query: str) -> dict[Any, Any]:
    ...
    try:
        ...
        if connection.is_connected():
            cursor = connection.cursor(buffered=True)  # Use buffered cursor to handle result sets properly
            
            # Split and execute multiple statements if they exist
            ...
            if query.strip().upper().startswith('SELECT'):
                ...
                connection.commit()
                # Get the total row count for the last executed statement
                result = {
                    "message": f"Query executed successfully. Rows affected: {cursor.rowcount}",
                    "success": True
                }
                
            # Consume any remaining results to prevent "Commands out of sync" error
            try:
                while cursor.nextset():
                    pass  # Consume any additional result sets
            except:
                pass  # Ignore errors when consuming additional result sets
            
    except Error as e:
        result = {"message": f"Database error: {str(e)}", "success": False}
    except Exception as e:
        result = {"message": f"Error: {str(e)}", "success": False}
    finally:
...
@mcp.tool()
def write_file(file_path: str, content: str) -> dict[Any, Any]:
    """
    ...
    try:
        with open(file_path, 'a', encoding='utf-8') as f:
            f.write(content)
        return {"message": f"Successfully appended to {file_path}", "success": True}
    except Exception as e:
        return {"message": f"Error writing to file: {str(e)}", "success": False}
